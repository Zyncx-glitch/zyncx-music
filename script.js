// ZYNCX CORE ENGINE v3.2 - Edición Estable de Búsqueda
...
async function buscarMusica() {
    ...
    resultsContainer.innerHTML = `
        <div class="text-center py-10">
            ...
            <p class="text-cyan-400 font-mono text-[10px] uppercase tracking-widest">Zyncx Search: Conectando Red...</p>
        </div>`;

    try {
        // Buscador ultra-limpio usando el motor de sugerencias de DuckDuckGo vía Proxy CORS
        const targetUrl = `https://duckduckgo.com/ac/?q=${encodeURIComponent(queryInput)}&type=list`;
        const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(targetUrl)}`;
        
        const response = await fetch(proxyUrl);
        if (!response.ok) throw new Error("Proxy ocupado");
        
        const wrapper = await response.json();
        // DuckDuckGo devuelve un array de strings directo dentro de un JSON perfecto
        const sugerencias = JSON.parse(wrapper.contents)[1] || [];

        resultsContainer.innerHTML = "";

        if (sugerencias.length > 0) {
            sugerencias.slice(0, 6).forEach(tituloTerm => {
                const cleanTitle = tituloTerm.replace(/['"]/g, "");
                ...
                <div class="glass p-4 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-all cursor-pointer mb-3 group" 
                     onclick="obtenerIDYDescargar('${cleanTitle}')">
                    <div class="flex items-center gap-4 text-left">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-500 font-mono uppercase tracking-widest">Zyncx Cloud Server</p>
                        </div>
                    </div>
            ...
            resultsContainer.innerHTML = `<p class="text-center text-gray-400 italic">No se encontraron pistas para "${queryInput}".</p>`;
        }
    } catch (error) {
        console.error("Error global de búsqueda:", error);
        resultsContainer.innerHTML = `<p class="text-red-400 text-center font-bold">Error de sincronización de datos. Reintenta de nuevo.</p>`;
    }
}
...
async function obtenerIDYDescargar(nombreTema) {
    ...
    resultsContainer.innerHTML = `
        <div class="text-center py-20">
            ...
            <h2 class="text-xl font-bold text-white uppercase tracking-widest mb-2">Generando Streaming</h2>
            <p class="text-cyan-400 text-xs animate-pulse">${nombreTema}</p>
        </div>`;

    try {
        // Enrutamos mediante AllOrigins a una API de búsqueda limpia de Invidious
        ...
        if (data && data.length > 0) {
            ...
            procesarEnlaceRespaldo(nombreTema);
        }
    } catch (e) {
        procesarEnlaceRespaldo(nombreTema);
    }
...
async function ejecutarDescargaRapidAPI(id, titulo) {
    ...
    try {
        ...
        procesarEnlaceRespaldo(titulo);
    }
}

function procesarEnlaceRespaldo(titulo) {
    const resultsContainer = document.getElementById('results');
    const backupUrl = `https://9xbuddy.com/process?url=https://www.youtube.com/results?search_query=${encodeURIComponent(titulo)}`;
    resultsContainer.innerHTML = `
        <div class="glass p-10 rounded-[2.5rem] border border-cyan-500/30 text-center max-w-md mx-auto shadow-2xl">
